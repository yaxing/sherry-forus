...
namespace DAL
{
    public class ShopCartInfoDAL
    {
        ...
        /// </summary>
        /// <param name="info">新商品实体对象</param>
        /// <param name="flag">电购标记</param>
        /// <returns>bool值</returns>
        public bool SetItemEntity(ref ItemEntity info, int flag)
        {
            //=====================================获取商品基本信息==================================//
            DataTable dt = new DataTable();
            sqlString = "select goodsName, goodsPrice, goodsImg from goodsInfo where goodsID=" + info.ID;
        ...
        #region 查询商品库存
        public static bool GetStoragePrice(ref int storage, ref double price, int id)
        {
            DataTable dt = new DataTable();
            sqlString = "select goodsStorage, goodsPrice from goodsInfo where goodsID=" + id;
            try
            {
                using (dp = new DataProvider())
                {
            ...
            storage = Convert.ToInt32(dt.Rows[0]["goodsStorage"].ToString());
            price = Convert.ToDouble(dt.Rows[0]["goodsPrice"].ToString());
            return true;
        }
        #endregion
    }
