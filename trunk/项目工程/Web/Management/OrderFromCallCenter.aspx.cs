...
public partial class Management_OrderFromCallCenter : System.Web.UI.Page
{
    ...
    protected void bConfirm_Click(object sender, EventArgs e)
    {
        ...
        {
            Response.Write("<script>alert('交易已完成');location.href('OrderFromCallCenter.aspx?ID=" + curOrder.OrderID + "');</script>");
            return;
        }
    }
    ...
    //    else
    //    {
    //        Response.Write("<script>alert('订单成功撤销！');location.href('OrderFromCallCenter.aspx');</script>");
    //        return;
    //    }
    //}
    ...
    protected void bBackToList_Click(object sender, EventArgs e)
    {
        ...
        if (state >= 0)
        {
            Response.Redirect("OrderFromCallCenter.aspx?OrderState=" + state);
        }
        else
        {
            Response.Redirect("OrderFromCallCenter.aspx");
        }
    }
}
